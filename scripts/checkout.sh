...
then
	...
	pushd $ROOTDIR/$MAIN/$RDIR
	   ...
	   echo ${row} | base64 --decode | jq --arg comhash "${comhash}" --arg toolchainhash "${toolchainhash}" '. + {documentcommit : $comhash, toolchaincommit: $toolchainhash, hostname: "https://semiceu.github.io" }' > .publication-point.json
	   cleanup_directory 
        popd
